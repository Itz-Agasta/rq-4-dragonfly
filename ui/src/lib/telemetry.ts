...
export interface TwinOutput {
  ...
  detection: Detection;
  /** Which fault the residual pattern points at. */
  diagnosis: Diagnosis;
}
...
/**
 * The structured residual matrix's verdict. Mirrors `signature::Diagnosis`.
 *
 * Index 0 is the null hypothesis and reads 1.0 on a healthy engine, because
 * isolation is gated on detection: asked which fault about a healthy engine the
 * matrix would name whichever one the noise lies nearest.
 */
export interface Diagnosis {
  /** Posterior over hypotheses, summing to one. */
  posterior: number[];
  /** Cosine between the observation and each signature, minus one to one. */
  match_score: number[];
...
/**
 * Hypothesis names, in the order the matrix carries them.
 *
 * Mirrors `signature::CATALOGUE` in `twin-core`, by hand. The signatures
 * themselves are generated by running the model and are not sent on the wire:
 * they are constant for a given engine, and `just signatures` prints them.
 */
